...
//    }
//
//    func createSession(completion: @escaping (MPGSDKResult<GatewayMap>) -> Void) {
//        issueRequest(path: "create_session", method: "POST", completion: completion)
//    }
//
//    func check3DSEnrollment(transaction: Transaction, redirectURL: String, completion: @escaping (MPGSDKResult<GatewayMap>) -> Void) {
...
//    }
//
//    func completeSession(transaction: Transaction, completion: @escaping (MPGSDKResult<GatewayMap>) -> Void) {
...
//    }
//
//    fileprivate func issueRequest(path: String, method: String, query: [URLQueryItem]? = nil, body: GatewayMap? = nil, completion: @escaping (MPGSDKResult<GatewayMap>) -> Void) {
...
//    }
//
//    fileprivate func responseHandler<T: Decodable>(_ completion: @escaping (MPGSDKResult<T>) -> Void) -> (Data?, URLResponse?, Error?) -> Void {
//        return { (data, response, error) in
//            if let error = error {
//                completion(MPGSDKResult.error(error))
//                return
//            }
//
//            guard let httpResponse = response as? HTTPURLResponse, (200..<300).contains(httpResponse.statusCode), let data = data else {
//                completion(MPGSDKResult.error(MerchantAPIError.failedRequest))
//                return
//            }
//
//
